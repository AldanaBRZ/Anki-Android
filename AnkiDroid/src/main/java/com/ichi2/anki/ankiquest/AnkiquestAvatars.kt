...
object AnkiquestAvatars {
    ...
    data class Account(
        ...
        val scope: String = MessageDigest.getInstance("SHA-256")
    ...
    fun account(): Account? {
        ...
        val user = (settings[Ankiquest.USER_KEY] as? String).orEmpty().trim()
    ...
    private fun requireResponse(captured: Account, response: Response) {
    ...
    internal suspend fun refresh(users: List<String>, expected: Account?) =
        withContext(Dispatchers.IO) {
            mutex.withLock {
                val account = expected ?: run {
                    cached = Cache("", emptyMap())
                    nextDownload = 0
                    return@withLock
                }
